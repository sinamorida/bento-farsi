...
shipped app, and so are **bento/spaces** (Notion/notes-like),
**bento/dash** (spreadsheet + tables) and **bento/type** (word processor).
**bento/vault** (document library) is still only a plan.
...
6. **New UI strings go into ALL i18n catalogs.** English-string-as-key; never
   ...
   including this file.** It is `ls <app>/src/i18n/` — and since 2026-09-17
   the lists are DERIVED from that directory, not restated (slides'
   `build-i18n.mjs` still carries its own copy; it errors on a catalogue
   missing from it). This rule said seven for a month after Portuguese made it
   eight, and **bento/dash did ship without Portuguese** because a fourth copy
   of the list still said seven. Fixed by derivation: dash's rig/generator,
   `build-spaces-i18n.mjs` and `build-type-i18n.mjs` now read their locale
   columns from the catalog directory itself — adding a language is adding a
   catalog file, full stop.
   - **A generator that grades its own output cannot report a gap in the set
     it was given.** dash's rig IS its generator, so a restated list in it was
     invisible to every check; the derivation removed the set, not the gap.
   - **A rig must derive the list, never restate it.**
     `scripts/test-i18n-coverage.mjs` reads `PACKED_LOCALES` out of the
     generated `packed.ts`; the other apps' rigs list the catalog directory.
     **It covers slides ONLY** (`coreDir` is hardcoded to `slides/src/i18n`),
     so do not assume your app's catalogues are checked by anything beyond its
     own rig.
